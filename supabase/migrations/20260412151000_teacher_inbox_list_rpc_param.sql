-- PostgREST يتعرّف بشكل أوثق على دوال ذات معاملات؛ إضافة p_workspace_id (اختياري، لا يُستخدم في المنطق حالياً)
-- + إشعار لإعادة تحميل كاش المخطط بعد النشر.

drop function if exists public.teacher_inbox_list ();

create or replace function public.teacher_inbox_list (p_workspace_id uuid default null)
returns table (
...
grant execute on function public.teacher_inbox_list (uuid) to authenticated;

comment on function public.teacher_inbox_list (uuid) is
'Inbox rows for workspace owner and linked staff (teacher_staff). p_workspace_id reserved for future filtering; pass null.';
