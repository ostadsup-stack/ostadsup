-- Restore explicit-ack path for same teacher overlapping two groups (parallel to slot-request consent for same-group).
...
comment on column public.schedule_events.teacher_cross_group_overlap_ack is
  'When true, insert/update allowed despite overlapping times for same created_by across different groups (teacher explicit consent).';
