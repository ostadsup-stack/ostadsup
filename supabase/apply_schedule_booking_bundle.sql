-- =============================================================================
-- تطبيق يدوي في Supabase → SQL Editor (مرة واحدة لكل مشروع)
-- 1) شغّل أولاً: supabase/queries/audit_schedule_overlaps.sql
--    إن ظهرت صفوف، أصلِح التداخلات قبل المتابعة.
-- 2) الصق هذا الملف كاملاً ونفّذه.
-- إن وُجد جدول schedule_slot_requests مسبقاً، سيفشل "create table" — احذف ذلك
-- القسم أو استخدم migrations عبر CLI بدلاً من هذا الملف.
-- =============================================================================

-- === من 20260411160000_schedule_overlap_slot_requests.sql ===
...
create index if not exists schedule_slot_requests_group_id_idx on public.schedule_slot_requests (group_id);

create index if not exists schedule_slot_requests_requester_idx on public.schedule_slot_requests (requester_id);

drop index if exists schedule_slot_requests_one_pending_per_block;
...
drop policy if exists ssr_update_requester_cancel on public.schedule_slot_requests;
...
-- === من 20260412090000_schedule_events_overlap_trigger.sql ===
